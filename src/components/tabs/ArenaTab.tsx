    ...
    <div className="w-full flex flex-col p-3 bg-neutral-950 space-y-3.5 pb-28">
        ...
        {worlds.map((w) => {
          ...
          return (
            <button
              ...
              className={`px-3.5 py-2 rounded-2xl text-xs font-black shrink-0 flex items-center gap-1.5 border transition-all active:scale-95 shadow-md ${
                isSelected
                  ? 'game-btn-gold text-black border-amber-300'
                  : isWorldUnlocked
                    ? 'game-card text-zinc-300 border-white/10 hover:border-zinc-600'
                    : 'bg-neutral-950 text-zinc-600 border-zinc-900'
              }`}
            >
              {!isWorldUnlocked ? (
      ...
      <div className="game-card rounded-2xl p-3.5 shadow-xl border border-white/10">
        <h3 className="text-xs font-black text-amber-400 uppercase tracking-wider">
          {t('world', state.lang)} {selectedWorld}: {worlds[selectedWorld - 1].name}
        </h3>
      ...
      <div className="space-y-2">
        {Array.from({ length: 10 }, (_, i) => i + 1).map((subNum) => {
          ...
          return (
            <div
              key={subNum}
              className={`rounded-2xl border p-3 flex items-center justify-between transition-all shadow-md ${
                isCurrent
                  ? 'game-card-gold border-amber-500 shadow-amber-500/10'
                  : isUnlocked
                    ? 'game-card border-white/5 hover:border-white/15'
                    : 'bg-neutral-950/60 border-zinc-900 opacity-60'
              }`}
            >
              {/* Left Stage Details */}
              <div className="flex items-center gap-3">
                <div className={`w-11 h-11 rounded-2xl flex items-center justify-center font-black text-xs border shadow-inner ${
                  stageData.isBoss
                    ? 'game-btn-crimson text-white border-rose-400 animate-pulse'
                    : isCurrent
                      ? 'game-btn-gold text-black border-amber-300'
                      : 'bg-neutral-900 border-white/10 text-zinc-300'
                }`}>
                  {stageData.isBoss ? <Skull className="w-5 h-5 text-white" /> : `${selectedWorld}-${subNum}`}
                </div>
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {stageData.isBoss && (
                      <span className="px-1.5 py-0.2 bg-rose-600 text-white font-black text-[9px] rounded-md uppercase">
                        {t('boss', state.lang)}
                      </span>
                    )}
                  </div>
              ...
              {isUnlocked ? (
                <button
                  ...
                  className={`px-3.5 py-2 rounded-xl font-black text-xs flex items-center gap-1.5 transition-all active:scale-95 shadow-md ${
                    isCurrent
                      ? 'game-btn-gold text-black border-amber-300 animate-pulse'
                ...
                <div className="p-2.5 rounded-xl bg-neutral-900 border border-zinc-800 text-zinc-600">
